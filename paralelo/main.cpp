...
int main(int argc, char** argv) {
    ...
    cout << "Inicia programa paralelo" << endl;
    cout << "Ejecutando consultas" << endl;
    ...
    matrizListaDocentesHorario = getQuery("SELECT d.id, h.id \
                                            FROM cursos c INNER JOIN docentes d \
    ...
    // 1.- iterar listaHorarioDocente para calcular el horario de los docentes que tienen horario->libre = TRUE
...
#pragma omp parallel shared(matrizDocentes, matrizDisponibilidadDocentes, matrizDisponibilidadSalas, vectorStringDocente_id, matrizListaDocentesHorario) private (num_nodo, nodos_totales)
{

    num_nodo = omp_get_thread_num();
    nodos_totales = omp_get_num_threads();
